...
GenericLinkService::encodeLpFields(const ndn::PacketBase& netPkt, lp::Packet& lpPacket)
{
  ...
  //CHANGED
  shared_ptr<lp::MaxHopCountTag> maxHopCountTag = netPkt.getTag<lp::MaxHopCountTag>();
  if (maxHopCountTag != nullptr) {
    lpPacket.add<lp::MaxHopCountTagField>(*maxHopCountTag);
  }
  else {
    lpPacket.add<lp::MaxHopCountTagField>(0);
  }
  //CHANGED
                                   ...
                                   const EndpointId& endpointId)
{
  ...
  //CHANGED
  // Increment MaxHopCount
  if (firstPkt.has<lp::MaxHopCountTagField>()) {
    interest->setTag(make_shared<lp::MaxHopCountTag>(firstPkt.get<lp::MaxHopCountTagField>() + 1));
  }
  //CHANGED
                               ...
                               const EndpointId& endpointId)
{
  ...
  //CHANGED
  if (firstPkt.has<lp::MaxHopCountTagField>()) {
    data->setTag(make_shared<lp::MaxHopCountTag>(firstPkt.get<lp::MaxHopCountTagField>()));
  }
  //CHANGED
